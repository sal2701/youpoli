...
import { ChatAlt2Icon, PencilAltIcon, FireIcon} from '@heroicons/react/outline';
...
import { CreatePublicCommentRequest, useAddProfileInterestMutation } from 'lens';
import {
...
const NewPublication: FC<Props> = ({ publication }) => {
  ...
  //   onCompleted: ({ createSetProfileMetadataTypedData }) => typedDataGenerator(createSetProfileMetadataTypedData),
  //   onError
  // });

  const [] = useAddProfileInterestMutation({
  ...
  const createPublication = async () => {
    ...
    try {
      ...
      if (currentProfile?.dispatcher?.canUseRelay) {
        console.log("Relay used")
        await createViaDispatcher(request);
      } else {
        console.log("Relay not used")
        if (isComment) {
  ...
  const createPromotedPublication = async () => {
    if (!currentProfile) {
      return toast.error(SIGN_WALLET);
    }
    ...
    try {
      ...
      if (publicationContent.length === 0 && attachments.length === 0) {
        return setPublicationContentError(`${isComment ? 'Promoted Comment' : 'Promoted Post'} should not be empty!`);
      }
      ...
      const id = await uploadToArweave({
        ...
        image: attachments.length > 0 ? getAttachmentImage() : textNftImageUrl,
        imageMimeType: attachments.length > 0 ? getAttachmentImageMimeType() : 'image/svg+xml',
        name: isAudioPublication ? audioPublication.title : `Promoted Comment by @${currentProfile?.handle}`,
        tags: getTags(publicationContent),
        animation_url: getAnimationUrl(),
        mainContentFocus: getMainContentFocus(),
        contentWarning: null,
      ...
      <div className="block items-center sm:flex px-5">
        ...
        <div className="ml-auto pt-2 sm:pt-0">
          ...
          <Button
              ...
              onClick={createPromotedPublication}
            >
              {isComment ? 'Promote Comment' : 'Promote Post'}
            </Button>
            <Button
              disabled={isSubmitting}
              icon={
